...
##### Generic JSON and URL query creator 
This is the query creator that is shipped and is very flexible at being able to construct requests with json payloads, without
requiring you to write any java. The generic JSON and URL query creator defines configuration to map the SQL columns to the lookup query json request.
The configuration allows you to provide a :
- List of top level column names to be included in the query parameters.
- List of top level column names to be included in the body (for PUT and POST).
- Map of templated URL segment names to top level column names.

This query creator works with the JSON format or a customized version of the JSON format. 

When using this query creator, you should ensure that the incoming event values match the http connector table column
names and types. You can achieve this using a view prior to the lookup join to change column values and types appropriately.
The view can also be used to introduce constant columns - columns defined with constants e.g. literals for String columns. 
...
| gid.connector.http.request.query-param-fields                 | optional | Used with `gid.connector.http.source.lookup.query-creator` = `generic-json-url` Specifies the names of the top level column names that will be mapped to query parameters. The column names should be supplied as a string separated by semicolons, such as 'param1;param2'."                                                                                                                                                                                                                                                                                                                                                                                                                                |
| gid.connector.http.request.body-fields                        | optional | Used with `gid.connector.http.source.lookup.query-creator` = `generic-json-url` Specifies the names of the top level column names that will be mapped to the request body. The column names should be supplied as a string separated by semicolons, such as 'param1;param2'."                                                                                                                                                                                                                                                                                                                                                                                                                                
| gid.connector.http.request.url-map                            | optional | Used with `gid.connector.http.source.lookup.query-creator` = `generic-json-url` The map of insert names to top level column names, used in a templated url. This allows a templated URL with segments surrounded by curly braces to be amended at lookup query time to include specific column values. The expected format of the map is: `key1:value1,key2:value2`.  For example if there are table columns called customerId and orderId, then specifying value cid1:customerId,oid:orderID and a templated url of https://myendpoint/customers/{cid}/orders/{oid} will mean that the url used for the lookup query will dynamically pickup the values for customerId, orderId and use them in the url. |
