...
/**
 * Generic JSON and url query creator factory defines configuration to map the columns defined
 * in the SQL Table definition to the lookup query. These are:
 * <ol>
 *     <li>List of column names to be included in the query parameters</li>
...
public class GenericJsonAndUrlQueryCreatorFactory implements LookupQueryCreatorFactory {
    ...
    public static final ConfigOption<List<String>> REQUEST_QUERY_PARAM_FIELDS =
            key("gid.connector.http.request.query-param-fields")
                    ...
                    .withDescription(
                            "The names of the top level column names that will be mapped to query"
                            + " parameters. The parameters are separated by semicolons,"
    ...
    public static final ConfigOption<List<String>> REQUEST_BODY_FIELDS =
            key("gid.connector.http.request.body-fields")
                    ...
                    .defaultValues()   //default to empty list so we do not need to check for null
                    .withDescription(
                            "The names of the top level column names that will be mapped to the"
                                    + " body. The parameters are separated by semicolons,"
                                    + " such as 'param1;param2'.");
    public static final ConfigOption<Map<String, String>> REQUEST_URL_MAP =
            ConfigOptions.key("gid.connector.http.request.url-map")
                         .mapType()
                         .noDefaultValue()
                         .withDescription("The map of insert names to to level column names used"
